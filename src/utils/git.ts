...
export class Git {
  ...
  constructor (
    ...
    this.baseDir = join(process.cwd(), 'git-workspaces', owner, repository, '' + issueNumber)
    mkdirSync(this.baseDir, { recursive: true })
    this.gitClient = simpleGit({
      baseDir: this.baseDir,
    })
    if (isProduction()) {
      this.gitClient.raw(['config', '--global', 'user.name', process.env.APP_NAME || ''])
      this.gitClient.raw(['config', '--global', 'user.email', process.env.GIT_USER_EMAIL || ''])
      this.gitClient.raw(['config', '--global', 'push.default', 'simple'])
    }
  }

  async pull (branch: string, remote: string = 'origin') {
  ...
  async push (force: boolean = false) {
    ...
    return this.gitClient.push(undefined, undefined,
      option,
    )
  }
